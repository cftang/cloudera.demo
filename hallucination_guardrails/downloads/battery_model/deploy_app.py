#!/usr/bin/env python3
"""
Deploy the Story 01 Battery Quality Prediction Service as a CAI Application.
...
BASE_APP_NAME = "Battery Quality Prediction Service"
PROJECT_ROOT = "/home/cdsw"
DEFAULT_APP_SCRIPT = f"{PROJECT_ROOT}/battery_model/run_app.py"
RUNTIME_IMAGE = (
...
def get_subdomain(project_id: str) -> str:
    suffix = os.environ.get("app_suffix", "").strip()
    base = f"battery-quality-{project_id.lower()}"
    return f"battery-quality-{suffix.lower()}-{project_id.lower()}" if suffix else base


def main() -> None:
    print("=" * 60)
    print("  Deploy Battery Quality Prediction Service as CAI Application")
    print("=" * 60)
    ...
    app_config = {
        "name": app_name,
        "description": "Story01 Battery Pack QA — defect/risk prediction REST endpoint for Agent Studio",
        "subdomain": subdomain,
        "script": app_script,
        "kernel": "python3",
        "cpu": 2,
        "memory": 4,
        # Auth-required by default: many workspaces forbid unauthenticated apps site-wide
        # (HTTP 400 "cannot bypass authentication in this application"). Set BYPASS_AUTH=true
        # only if your admin allows it.
        "bypass_authentication": os.environ.get("BYPASS_AUTH", "false").lower() == "true",
    ...
    try:
        ...
        print('       -d \'{"machine_id":"PACK-07","process_type":"module_assembly",')
        print('             "temperature":32.2,"temperature_delta":3.9,"voltage_std":0.021,')
        print('             "press_force":3.05,"dcr":0.86,"cell_lot":"LOT-2026-0619"}\'')
